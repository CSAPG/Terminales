...
# Chapitre 18 Exercice 40                                   CORRECTION
...
t= np.linspace(0,4*T,400)  # 400 dates régulièrement espacées sur 4 périodes
...
def interference(A1,A2,phi):
    s1,= plt.plot(t,s(A1,0),'--',lw=1.2,label='$s_{1}(t)$') # Trace le signal s1(t) = A1*cos(2πt/T)
    s2,= plt.plot(t,s(A2,phi),'--',lw=1.2,label='$s_{2}(t)$') # Trace le signal s2(t) = A2*cos(2πt/T+phi)
    S,= plt.plot(t,s(A1,0)+s(A2,phi),lw=1.5,label='$s_{1}(t)+s_{2}(t)$')  # Trace le signal somme S(t) = s1(t)+s2(t)
    z,= plt.plot(t,(s(A1,0)+s(A2,phi))**2,lw=1.5,label='$(s_{1}(t)+s_{2}(t)^{2}$')  # Trace le carré de la somme (s1(t)+s2(t))^2
